        ...
        public void Draw(SpriteBatch spriteBatch, Player player, Level level)
        {
            for (int i = 0; i < level.YRList.Count; i++)
            {
                ...
                else if (level.YRList[i].isOn && level.YRList[i].activatedBy == this)
                {
                    ...
                    spriteBatch.Draw(level.Textures[18], start, null, Color.White, angle, new Vector2(0, level.Textures[5].Height / 2f), new Vector2(length / level.Textures[5].Width, 1f), SpriteEffects.None, 0f);
                }
            }
